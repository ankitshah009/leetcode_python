...
#buy more tickets. If a person does not need to buy any more tickets, the person will leave
#the line.
#
...
class Solution:
    def timeRequiredToBuy(self, tickets: List[int], k: int) -> int:
        ...
        for i in range(len(tickets)):
            if i <= k:
                # People before or at position k buy min(their tickets, target)
                time += min(tickets[i], target)
            else:
                # People after position k buy min(their tickets, target - 1)
                # because they won't get a turn after person k is done
