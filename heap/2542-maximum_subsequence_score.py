...
#    elements from nums2.
#    It can be defined simply as: (nums1[i0] + nums1[i1] +...+ nums1[ik - 1]) * min(nums2[i0],
#    nums2[i1], ... ,nums2[ik - 1]).
#
#Return the maximum possible score.
#
#A subsequence of indices of an array is a set that can be derived from the set {0, 1, ...,
#n-1} by deleting some or no elements.
#
#Example 1:
...
class Solution:
    def maxScore(self, nums1: List[int], nums2: List[int], k: int) -> int:
        ...
        # This way, as we iterate, the current nums2 value is always the minimum
