...
#average is (100 + 97 + 93 + 77 + 76) / 5 = 88.
#
#Example 2:
...
class Solution:
    def highFive(self, items: List[List[int]]) -> List[List[int]]:
        """
        Group by student ID, keep top 5 scores for each.
        """
...
class SolutionHeap:
    def highFive(self, items: List[List[int]]) -> List[List[int]]:
        ...
        return [[sid, sum(scores[sid]) // 5] for sid in sorted(scores.keys())]
...
class SolutionNLargest:
    def highFive(self, items: List[List[int]]) -> List[List[int]]:
        ...
        return [[sid, sum(heapq.nlargest(5, scores[sid])) // 5]
