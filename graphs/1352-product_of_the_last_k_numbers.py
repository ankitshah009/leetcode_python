...
class ProductOfNumbers:
    """
    Use prefix products. Reset when 0 is added.
    prefix[i] = product of all elements from last 0 to index i.
    """

    def __init__(self):
        self.prefix = [1]  # Start with 1 for easy calculation

    def add(self, num: int) -> None:
        if num == 0:
            # Reset prefix products
            self.prefix = [1]
        else:
    ...
    def getProduct(self, k: int) -> int:
        if k >= len(self.prefix):
            # There's a 0 in the last k elements
            return 0
        return self.prefix[-1] // self.prefix[-k - 1]


class ProductOfNumbersBruteForce:
    """Naive O(k) per query approach"""
