...
class Solution:
    def minReorder(self, n: int, connections: List[List[int]]) -> int:
        ...
        # (neighbor, needs_reorder): needs_reorder = 1 if edge goes away from 0
        graph = defaultdict(list)

        for a, b in connections:
            graph[a].append((b, 1))  # Original direction: a -> b (needs reorder if going away from 0)
            graph[b].append((a, 0))  # Reverse direction: b -> a (doesn't need reorder)
        ...
        while queue:
            ...
            for neighbor, cost in graph[city]:
                if neighbor not in visited:
                    ...
                    changes += cost  # Add cost if edge needs to be reoriented
