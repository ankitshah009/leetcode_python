...
#status, candies, keys, and containedBoxes.
#
#status[i] is 1 if the ith box is open and 0 if it is closed.
...
#can use the keys in it to open new boxes and you can also use the boxes you
#find in it.
#
...
class Solution:
    def maxCandies(self, status: List[int], candies: List[int], keys: List[List[int]],
        ...
        """
        BFS simulation. Keep track of boxes we have and keys we have.
        """
        n = len(status)
        have_box = [False] * n
        have_key = [bool(s) for s in status]  # Initially open boxes = have key
        opened = [False] * n
        ...
        while queue:
            ...
            for contained in containedBoxes[box]:
                have_box[contained] = True
                if have_key[contained] and not opened[contained]:
...
class SolutionIterative:
    def maxCandies(self, status: List[int], candies: List[int], keys: List[List[int]],
        ...
        have_keys = {i for i, s in enumerate(status) if s == 1}
        opened = set()
        ...
        while changed:
            ...
            for box in list(have_boxes):
                if box not in opened and box in have_keys:
                    opened.add(box)
                    total += candies[box]
