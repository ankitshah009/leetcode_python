...
#Input: root = [3,0,0]
#Output: 2
#Explanation: From the root of the tree, we move one coin to its left child, and one coin
#to its right child.
#
#Example 2:
#Input: root = [0,3,0]
...
class Solution:
    def distributeCoins(self, root: Optional[TreeNode]) -> int:
        ...
        def dfs(node):
            ...
            # Each coin moved across an edge counts as one move
            self.moves += abs(left_excess) + abs(right_excess)

            # Return this subtree's excess/deficit
            # node.val - 1 = excess coins at this node (negative means deficit)
