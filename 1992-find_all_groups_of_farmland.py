...
#meaning farmland in one group is not four-directionally adjacent to another farmland in
#another group.
#
...
#in land. If there are no groups of farmland, return an empty array. You may return the
#answer in any order.
#
#Example 1:
...
class Solution:
    def findFarmland(self, land: List[List[int]]) -> List[List[int]]:
        ...
        for i in range(m):
            for j in range(n):
                if land[i][j] == 1:
                    # Found top-left corner of a group
                    r2, c2 = i, j
